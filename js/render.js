function renderTemplateEditor(tpl){
  ...
  h+="<button onclick=\"duplicateTemplate('"+tpl.id+"')\" title=\"Duplizieren\">ð Duplizieren</button>";
  h+="<button onclick=\"tplAddPhase('"+tpl.id+"')\" title=\"Phase hinzufÃ¼gen\">ï¼ Phase</button>";
  h+="<button class=\"danger\" onclick=\"deleteTemplate('"+tpl.id+"')\" title=\"LÃ¶schen\">ð LÃ¶schen</button>";
  h+="</div>";
  ...
  for(let pi=0;pi<phases.length;pi++){
    ...
    h+="<button class=\"tpl-del\" onclick=\"event.stopPropagation();tplDelPhase('"+tpl.id+"',"+pi+")\" title=\"Phase lÃ¶schen\">â</button>";
    h+="</div>";
    ...
    for(let ki=0;ki<pkgs.length;ki++){
      ...
      h+="<button class=\"tpl-del\" onclick=\"tplDelPkg('"+tpl.id+"',"+pi+","+ki+")\" title=\"Package lÃ¶schen\">â</button>";
      h+="</div>";
      ...
      for(let ti=0;ti<tasks.length;ti++){
        ...
        h+="<button class=\"tpl-del\" onclick=\"tplDelTask('"+tpl.id+"',"+pi+","+ki+","+ti+")\" title=\"Task lÃ¶schen\">â</button>";
        h+="</div>";
      }
      h+="<button class=\"tpl-add-btn\" onclick=\"tplAddTask('"+tpl.id+"',"+pi+","+ki+")\" >ï¼ Task</button>";
      h+="</div>";
    }
    h+="<button class=\"tpl-add-btn\" onclick=\"tplAddPkg('"+tpl.id+"',"+pi+")\" style=\"margin-top:4px\">ï¼ Package</button>";
    h+="</div></div>";
  }
  h+="</div>";
  return h;
...
function renderProcessDashboard(){
  ...
  if(!m){ el.innerHTML='<div class="pi-empty">Kein aktives Projekt ausgewÃ¤hlt.</div>'; return; }
  let h = '<div class="pi-dash">';
  h += '<div class="pi-controls">';
  h += '<span style="font-weight:600;font-size:15px">ð Process Intelligence â '+m.projName+'</span>';
  ...
  h += '<button class="pi-export-btn" onclick="exportProcessReport()">ð Export</button>';
  h += '</div>';
  // KPI cards
  ...
  if(m.overPlan.length){
    h += '<div class="pi-section"><h3>â ï¸ Ãber-Plan Tasks</h3>';
  ...
  if(m.underPlan.length){
    h += '<div class="pi-section"><h3>â Effiziente Tasks</h3>';
  ...
  if(m.ownerList.length){
    h += '<div class="pi-section"><h3>ð¥ Mitarbeiter-Auslastung</h3>';
  ...
  if(m.noOwner.length){
    h += '<div class="pi-section"><h3>â±ï¸ Verwahrloste Tasks (ohne Owner)</h3>';
  ...
  el.innerHTML=DB.clients.map(c=>{
    ...
    let h=`<div class="ci-group${isActive?' ci-group-active':''}">
      <div class="client-item${isActive&&!isExpanded?' active':''}" onclick="toggleClientExpand('${c.id}',event)">
        ...
        <span class="ci-del" onclick="delClient('${c.id}',event)">â</span>
      </div>`;
    if(isExpanded){
      ...
      c.projects.forEach(proj=>{
        ...
        const typeIcon=proj.type==='retainer'?'ð':proj.type==='empty'?'ð':'ð';
        h+=`<div class="proj-item${isActiveProj?' active':''}${isCompleted?' completed':''}" onclick="switchProject('${c.id}','${proj.id}',event)">
          ...
          <span class="proj-del" onclick="delProject('${c.id}','${proj.id}',event)">â</span>
        </div>`;
      });
...
function renderTopbar(){
  ...
  const projLabel=client?client.name+' â '+c.name:c.name;
  document.getElementById('clientTitle').textContent=projLabel;
  document.title=`LaunchRamp â ${projLabel}`;
  ...
  const qd=[
    {key:'homepage',label:'Homepage',icon:'ð'},
    {key:'instagram',label:'Instagram',icon:'ð¸'},
    {key:'linkedin',label:'LinkedIn',icon:'ð¼'},
    {key:'gdrive',label:'Google Drive',icon:'ð'},
    {key:'claude',label:'Claude Projekt',icon:'ð¤'}
  ];
  if(c.customLinks)c.customLinks.forEach((cl,i)=>qd.push({key:'custom_'+i,label:cl.label,icon:'ð',custom:true,idx:i}));
  ...
  document.getElementById('linkPanel').innerHTML='<h4>Projekt-Links</h4>'+qd.map(q=>{
    ...
    return '<div class="link-item '+cls+'" onclick="'+(url?"window.open(\'"+esc(url)+"\',\'_blank\')":"editQuickLink(\'"+(q.custom?"custom_"+q.idx:q.key)+"\')")+'"><span class="li-icon">'+q.icon+'</span><span class="li-label">'+esc(q.label)+'</span><span class="li-status">'+(url?'â Aktiv':'Nicht gesetzt')+'</span></div>';
  }).join('')+'<div class="link-add" onclick="addCustomLink()"><span>+</span><span>Link hinzufÃ¼gen</span></div>'
}
...
function renderStats(){
  ...
  document.getElementById('sHL').textContent=Math.round((tm-dm)/60)+'h Ã¼brig';
...
const wKey=`${pi}_${pai}`;
      ...
      <div class="wp-head" onclick="toggleW('${phObj._id}','${pkg._id}')">
        <span class="drag-handle" title="Kategorie verschieben" onmousedown="event.stopPropagation()">â ¿</span>
        <span class="wp-chev${wOpen?' open':''}" id="wc${pi}_${pai}">â¶</span>
        <span class="wp-name">${esc(pkg.name)}</span>${pkgTimeHtml}
        <span style="font-size:10px;color:var(--text3);margin-left:6px">${pdd}/${ptt}</span>
        ${ppct===100?'<span style="font-size:9px;color:var(--green);margin-left:4px">â</span>':''}
        <button class="btn sm int-col" style="margin-left:auto;opacity:0;transition:opacity .15s" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0" onclick="event.stopPropagation();renamePackage('${phObj._id}','${pkg._id}')">âï¸</button>
        <button class="btn sm int-col danger" style="opacity:0;transition:opacity .15s;font-size:9px" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0" onclick="event.stopPropagation();deletePackage('${phObj._id}','${pkg._id}')">â</button>
...
// M-2: Extracted from renderTasks() â phase header block
...
let tt=0,dd=0;
    ...
    const dateStr=phase.startDate&&phase.endDate?`${fmtShort(phase.startDate)} â ${fmtShort(phase.endDate)}`:'';
    ...
    return `<div class="phase-block" data-pi="${pi}" draggable="true" ondragstart="dragPhaseStart(event,'${phase._id}')" ondragend="dragPhaseEnd(event)" ondragover="dragPhaseOver(event,'${phase._id}')" ondragleave="dragPhaseLeave(event)" ondrop="dropPhase(event,'${phase._id}')">
      <div class="phase-head" onclick="toggleP('${phase._id}')" style="border-left:3px solid ${phaseBarInfo.color}">
        <div class="phase-left">
          <span class="drag-handle" title="Phase verschieben" onmousedown="event.stopPropagation()">â ¿</span>
          <span class="ph-chev${pOpen?' open':''}" id="pc${pi}">â¶</span>
        ...
        <div class="phase-right">
          ...
          <div class="ph-actions" style="display:flex;gap:3px;align-items:center">
            ...
            <button class="btn sm" onclick="event.stopPropagation();editPhase('${phase._id}')" title="Phase bearbeiten">âï¸</button>
            <button class="btn sm int-col" onclick="event.stopPropagation();duplicatePhase('${phase._id}')" title="Phase duplizieren">ð</button>
            <button class="btn sm danger int-col" onclick="event.stopPropagation();deletePhaseInline('${phase._id}')" title="Phase lÃ¶schen" style="font-size:10px">ðï¸</button>
          </div>
        </div>
      </div>
...
// M-2: Extracted from renderTasks() â single task row HTML
...
const id=task._id;
        ...
        const ci=st==='Erledigt'?'â':st==='In Arbeit'?'â':st==='Warte auf Kunde'?'â':'';
        ...
        if(dl){
          ...
          if(st==='Erledigt'){
            dlBadge=`<span class="dl-badge dl-done" onclick="editDeadline('${phObj._id}','${pkObj._id}','${task._id}',event)" style="cursor:pointer" title="Klick: Deadline Ã¤ndern">${dlStr}</span>`;
          }else if(daysLeft>1){
            dlBadge=`<span class="dl-badge dl-ok" onclick="editDeadline('${phObj._id}','${pkObj._id}','${task._id}',event)" style="cursor:pointer" title="Klick: Deadline Ã¤ndern">${dlStr}</span>`;
          }else if(daysLeft>=0){
            dlBadge=`<span class="dl-badge dl-warn" onclick="editDeadline('${phObj._id}','${pkObj._id}','${task._id}',event)" style="cursor:pointer" title="Klick: Deadline Ã¤ndern">${dlStr}</span>`;
          }else{
            dlBadge=`<span class="dl-badge dl-over" onclick="editDeadline('${phObj._id}','${pkObj._id}','${task._id}',event)" style="cursor:pointer" title="Klick: Deadline Ã¤ndern">${dlStr} (${Math.abs(daysLeft)}d Ã¼ber)</span>`;
          }
        }}
        ...
        const timerBtn=`<span class="timer-btn${isTimerOn?' running':''}" onclick="toggleTimer('${id}',event)" title="${isTimerOn?'Timer stoppen':'Timer starten'}">${isTimerOn?'â¹':'â¶'}${tracked?`<span class="tracked">${tracked}m</span>`:''}</span>`;
        const timeBtn=`<span class="time-btn" onclick="editTaskTime('${phObj._id}','${pkObj._id}','${task._id}')" title="Zeitbudget anpassen">â± ${mins}m</span>`;
        const sched=task.scheduled;
        const calBtn=sched
          ?`<span class="cal-btn cal-scheduled" onclick="openGCal('${phObj._id}','${pkObj._id}','${task._id}')" title="Zeitblock: ${fmtShort(sched.date)} ${sched.time}">ð ${fmtShort(sched.date)}</span>`
          :`<span class="cal-btn" onclick="openGCal('${phObj._id}','${pkObj._id}','${task._id}')" title="Zeitblock planen">ð</span>`;
        const promptBtn=pLink
          ?`<span class="link-btn has-link" onclick="window.open('${esc(pLink)}','_blank')" title="${esc(pLink)}">ð Prompt <span class="gear" onclick="event.stopPropagation();editTaskLink('${phObj._id}','${pkObj._id}','${task._id}','prompt')">â</span></span>`
          :`<span class="link-btn empty int-col" onclick="editTaskLink('${phObj._id}','${pkObj._id}','${task._id}','prompt')">ð Prompt</span>`;
        const sopBtn=sLink
          ?`<span class="link-btn has-link" onclick="window.open('${esc(sLink)}','_blank')" title="${esc(sLink)}">ð¬ SOP <span class="gear" onclick="event.stopPropagation();editTaskLink('${phObj._id}','${pkObj._id}','${task._id}','sop')">â</span></span>`
          :`<span class="link-btn empty int-col" onclick="editTaskLink('${phObj._id}','${pkObj._id}','${task._id}','sop')">ð¬ SOP</span>`;
      var noteIcon=task.notes?'<span class="note-icon" title="'+esc(task.notes)+'">ð</span>':'';
        return `<div class="task-row ${(task.status==='Offen'&&getDueClass(task)==='due-overdue')?'overdue-row':''} ${st==='Erledigt'?'done':''}" data-id="${id}" data-owner="${task.owner}" data-status="${st}" oncontextmenu="ctxShow(event,'${id}','${phObj._id}','${pkObj._id}','${task._id}')">
          ...
          ${dlBadge}
          <div class="task-actions int-col"><button class="task-action" onclick="editTask('${phObj._id}','${pkObj._id}','${task._id}')" title="Bearbeiten">âï¸</button></div>
        </div>`;
}
...
function renderTasks(){
  ...
  h+=`<div style="text-align:center;margin:12px 0 8px;display:flex;gap:8px;justify-content:center;flex-wrap:wrap">
    ...
    ${c.type==='retainer'?`<button class="add-btn" onclick="duplicateRetainer('${c.id}')" style="font-size:12px;padding:8px 18px;font-weight:600;border-color:var(--blue);color:var(--blue)">ð NÃ¤chsten Monat</button>`:''}
    <button class="add-btn" onclick="toggleProjectComplete()" style="font-size:11px;padding:6px 14px;opacity:.6">${c.completed?'â©ï¸ Reaktivieren':'â AbschlieÃen'}</button>
  </div>`;
  ...
  el.innerHTML=`
    <div style="display:flex;align-items:center;gap:8px;width:100%">
      <div style="position:relative;flex:1;height:22px;background:var(--surface3);border-radius:11px;overflow:hidden;min-width:120px">
        ...
        <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:var(--text);z-index:4;pointer-events:none;text-shadow:0 0 3px rgba(255,255,255,.8)">${taskPct}% erledigt Â· Soll ${Math.round(timePct)}%</div>
      </div>
    </div>`;
}
...
function renderDocs(){
  ...
  DOCS.forEach(d=>{
    ...
    h+=`<div class="doc-card ${has?'has-link':''} ${d.int?'int-doc':''}" onclick="openDocLink('${d.id}')">
      ...
      <button class="doc-link-btn" onclick="event.stopPropagation();editLink('${d.id}','${d.name}')" title="Link bearbeiten">â</button>
    </div>`;
  });
  h+='</div>';
  ...
  if(!isCV){
    ...
    h+=`<div class="jf-section"><div class="sec-title" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">ð Jourfix`;
    if(hasJF){
      if(jfLink){
        h+=`<a href="${esc(jfLink)}" target="_blank" style="font-size:11px;font-weight:600;color:var(--green);background:var(--green-bg);padding:2px 10px;border-radius:12px;text-decoration:none;cursor:pointer" title="Meeting Ã¶ffnen">${jfDay}, ${jfTime} Uhr</a>`;
      }else{
      ...
      h+=`<span style="font-size:10px;color:var(--text3);cursor:pointer" onclick="setupJourfix()" title="Jourfix Ã¤ndern">âï¸</span>`;
      if(!jfLink){
        h+=`<span style="font-size:10px;color:var(--text3);cursor:pointer;border-bottom:1px dashed var(--border2)" onclick="editJFLink()" title="Meeting-Link hinzufÃ¼gen">+ Link</span>`;
      }
    }else{
...
function renderDashboard(){
  ...
  DB.clients.forEach(c=>{
    ...
    clientCards.push({id:c.id,name:c.name,pct,total:cTotal,done:cDone,inProgress:cIP,waiting:cWait,open:cOpen,estMins:cEstMins,doneMins:cDoneMins,
      ...
      jf:jf&&jf.jourfix.day?`${dayNames[jf.jourfix.day]} ${jf.jourfix.time}`:'â'});
  });
  ...
  let h=`<div class="dash-header"><div>
    ...
    <div class="dash-subtitle">${DB.clients.length} Kunde${DB.clients.length!==1?'n':''} Â· ${new Date().toLocaleDateString('de-DE',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</div>
  </div></div>`;
  ...
  clientCards.forEach(c=>{
    ...
    h+=`<div class="dash-card" onclick="switchClient('${c.id}')">
      ...
      <div class="dash-card-meta">
        <span>â ${c.done}/${c.total}</span>
        <span>ð ${c.inProgress}</span>
        <span style="color:var(--pink)">â³ ${c.waiting}</span>
        <span>ð ${c.activeProjects||0} Projekte</span>
  ...
  if(allOverdue.length){
    h+=`<div class="dash-section">
      <div class="dash-section-title">ð´ ÃberfÃ¤llig <span class="badge" style="background:var(--red)">${allOverdue.length}</span></div>
      <table class="dash-table"><thead><tr><th>Kunde</th><th>Task</th><th>Phase</th><th>Owner</th><th>ÃberfÃ¤llig</th></tr></thead><tbody>`;
  ...
  if(allWaiting.length){
    h+=`<div class="dash-section">
      <div class="dash-section-title">â³ Warte auf Kunde <span class="badge" style="background:var(--pink)">${allWaiting.length}</span></div>
  ...
  if(allThisWeek.length){
    h+=`<div class="dash-section">
      <div class="dash-section-title">ð Diese Woche fÃ¤llig <span class="badge" style="background:var(--yellow)">${allThisWeek.length}</span></div>
  ...
  if(owners.length){
    h+=`<div class="dash-section">
      <div class="dash-section-title">ð¥ Workload pro Owner</div>
    ...
    owners.forEach(([name,d])=>{
      ...
      h+=`<div class="dash-owner-card">
        ...
        <div class="dash-owner-stats">
          <span>â ${d.done}/${d.total}</span>
          <span>ð ${d.active}</span>
          <span>â³ ${d.waiting}</span>
...
function renderDiffReview(changes) {
  ...
  var h = "<h3>ð¥ Import-Review</h3>";
  h += "<p>" + changes.length + " Ãnderungen: " + greens.length + " ð¢  " + yellows.length + " ð¡  " + reds.length + " ð´</p>";
  if (greens.length) h += "<button onclick=\"checkAllGreen()\" class=\"btn btn-sm\" style=\"margin-bottom:1rem\">â Alle GrÃ¼nen annehmen</button>";
  function row(c, i) {
    var chk = c.severity === "green" ? "checked" : "";
    var icon = c.severity === "green" ? "ð¢" : c.severity === "yellow" ? "ð¡" : "ð´";
    var desc = "";
    if (c.type === "status_change") desc = icon + " <b>" + esc(c.task) + "</b> â " + esc(c.oldStatus) + " â " + esc(c.newStatus);
    else if (c.type === "deadline_change") desc = icon + " <b>" + esc(c.task) + "</b> â Deadline: " + (c.oldDeadline||"â") + " â " + (c.newDeadline||"â");
    else if (c.type === "owner_change") desc = icon + " <b>" + esc(c.task) + "</b> â Owner: " + (c.oldOwner||"â") + " â " + (c.newOwner||"â");
    else if (c.type === "rename_task") desc = icon + " Umbenennung: " + esc(c.task) + " â " + esc(c.newName);
    else if (c.type === "notes_change") desc = icon + " <b>" + esc(c.task) + "</b> â Notiz aktualisiert";
    else if (c.type === "effort_change") desc = icon + " <b>" + esc(c.task) + "</b> â Aufwand: " + (c.oldMin||"â") + " â " + (c.newMin||"â");
    else if (c.type === "new_task") desc = icon + " NEUER TASK: <b>" + esc(c.data.t || c.data.text) + "</b> â " + esc(c.phase) + " / " + esc(c.package);
    else if (c.type === "new_package") desc = icon + " NEUES PACKAGE: <b>" + esc(c.data.name) + "</b> â " + esc(c.phase);
    else if (c.type === "new_phase") desc = icon + " NEUE PHASE: <b>" + esc(c.data.name) + "</b>";
    else if (c.type === "rename_phase") desc = icon + " Phase: " + esc(c.phase) + " â " + esc(c.newName);
    else if (c.type === "delete_task") desc = icon + " LÃSCHEN: <b>" + esc(c.task) + "</b> (" + esc(c.phase) + ")";
  ...
  if (greens.length) { h += "<h4>ð¢ Status/Notiz-Ãnderungen</h4>"; greens.forEach(function(c) { h += row(c, changes.indexOf(c)); }); }
  if (yellows.length) { h += "<h4>ð¡ Inhaltliche Ãnderungen</h4>"; yellows.forEach(function(c) { h += row(c, changes.indexOf(c)); }); }
  if (reds.length) { h += "<h4>ð´ Strukturelle Ãnderungen</h4>"; reds.forEach(function(c) { h += row(c, changes.indexOf(c)); }); }
  h += "<div style=\"display:flex;gap:1rem;margin-top:1.5rem\"><button onclick=\"applySelectedChanges()\" class=\"btn\" style=\"background:var(--green)\">ð¾ AusgewÃ¤hlte Ã¼bernehmen</button>";
  h += "<button onclick=\"closeModal('diffModal')\" class=\"btn\" style=\"background:var(--red)\">â Abbrechen</button></div>";
